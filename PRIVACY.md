...
**Hive Light collects nothing. Nothing leaves your machine.**

Hive Light is a local macOS menu-bar app. It has no backend, no servers, no analytics,
and no telemetry. It makes no network requests. The maintainer receives no data about you
or your usage.

## What data Hive Light touches, and where it stays

- **Session state files** (`~/.hive-light/sessions/`). The hook shim writes each Claude
...
Hive Light observes state produced by Claude Code, which is governed by
...
Hive Light itself is not a party to either.

## Contact

Hive Light is open source (Apache-2.0). Questions or concerns:
[github.com/fr1j0/hive-light/issues](https://github.com/fr1j0/hive-light/issues).
