...
final class SessionTests: XCTestCase {
    func test_session_roundTrips_throughJSON_withSnakeCaseKeys() throws {
        ...
        let decoded = try HiveLightJSON.decoder.decode(Session.self, from: data)
    ...
    func test_transcriptPath_roundTrips_andDefaultsNilOnOldJSON() throws {
        ...
        let data = try HiveLightJSON.encoder.encode(s)
        XCTAssertEqual(try HiveLightJSON.decoder.decode(Session.self, from: data).transcriptPath, "/t.jsonl")

        let old = #"{"session_id":"y","status":"idle","project":"p","cwd":"/p","updated_at":"1970-01-01T00:16:40Z"}"#
        let decoded = try HiveLightJSON.decoder.decode(Session.self, from: Data(old.utf8))
    ...
    func test_handoffStatus_roundTrips_throughJSON() throws {
        ...
        XCTAssertEqual(try HiveLightJSON.decoder.decode(Session.self, from: data), s)
    }
    ...
    func test_branch_roundTrips() throws {
        let s = Session(sessionID: "b1", status: .running, project: "p", cwd: "/x",
                        ...
                        branch: "feat/labels")
        let data = try HiveLightJSON.encoder.encode(s)
        let back = try HiveLightJSON.decoder.decode(Session.self, from: data)
    ...
    func test_sessionJSON_withoutBranchKey_decodes() throws {
        ...
        let s = try HiveLightJSON.decoder.decode(Session.self, from: Data(json.utf8))
    ...
    func test_model_roundTrips_andAbsentKeyDecodes() throws {
        ...
        let back = try HiveLightJSON.decoder.decode(Session.self,
                                                      from: HiveLightJSON.encoder.encode(s))
        ...
        XCTAssertNil(try HiveLightJSON.decoder.decode(Session.self, from: Data(json.utf8)).model)
    }
}
